...
- **Тестирование и отладка**: при разработке запускать `./run.sh --shell` для входа в контейнер, и запускать тестовые Python-скрипты **внутри контейнера**, а не на хосте. Это гарантирует правильное окружение (Deno, Playwright, пути).
