# Оценка лётности — как считаются очки и статусы

**Версия:** 2.0

---

## 1. Обзор (v2.0)

Для каждой локации автоматически вычисляется числовой **score** и текстовый **status**.
Статус определяет, стоит ли ехать в это место для XC closed route полёта.

### Ключевые изменения v2.0 относительно v1.0

| Аспект | v1.0 | v2.0 |
|--------|------|------|
| **Формула score** | `-3×crit - 2×base - 1×qual - 1×danger + 2×pos` | Base score от thermal window + deductions + bonuses |
| **Главный критерий** | Количество и тяжесть флагов | **Размер термического окна** (часы thermal activity) |
| **Вес позитивов** | +2 за каждый | +1 за каждый |
| **Hard rule: база** | Нет | base < 2000m MSL → max MAYBE |
| **Multi-model** | Нет | Per-model assessment: если модель → NO-GO → ухудшение |
| **Meteo-Parapente** | Нет | Интеграция: MP_STRONG_THERMALS / MP_WEAK_THERMALS |
| **Fallback chains** | Фиксированные ключи | Family-based (ICON D2→EU→Global, ECMWF 0.25→0.4) |

---

## 2. Статусы (от лучшего к худшему)

| Статус | Emoji | Значение | Когда (после hard rules) |
|--------|-------|----------|--------------------------|
| **STRONG** | 💚 | Big day — высокие шансы на 150+ closed | score > 4 |
| **GO** | 🟢 | Уверенный шанс на 100+ closed | score > 1 и ≤ 4 |
| **MAYBE** | 🟡 | Возможно при удачном стечении | score > −2 и ≤ 1 |
| **UNLIKELY** | 🟠 | Маловероятно | score > −5 и ≤ −2 |
| **NO-GO** | 🔴 | Нереалистично | score ≤ −5 или hard rules |
| **NO DATA** | ⚪ | Нет данных для оценки | tw_hours=0 и 0 флагов и 0 позитивов |

Финальная сортировка результатов: STRONG → GO → MAYBE → UNLIKELY → NO-GO → NO DATA.

---

## 3. Входные данные для оценки

### 3.1 Combined Hourly Profile

Merge из best-available модели каждого семейства (08:00–18:00 local).
Приоритет слоёв: `Best ICON > Best ECMWF > GFS` (подробнее в priorities.md §4).

GFS-only поля: `boundary_layer_height`, `lifted_index`, `convective_inhibition`.

### 3.2 Расчётные поля (per hour)

| Поле | Формула | Зависимости |
|------|---------|-------------|
| `cloudbase_msl` | `125 × (T_2m − Td_2m) + elev` | temperature_2m, dewpoint_2m, location.elev |
| `lapse_rate` | `(T_850 − T_700) / 1.5` °C/km | temperature_850hPa, temperature_700hPa |
| `wstar` | Deardorff: `(g/T_K × BL_h × 0.4·SWR / (1.1·1005))^(1/3)` | boundary_layer_height (GFS), shortwave_radiation, temperature_2m |
| `gust_factor` | `windgusts_10m − windspeed_10m` м/с | windgusts_10m, windspeed_10m |

### 3.3 Параметры локации

| Поле | Описание | Пример |
|------|----------|--------|
| `elev` | Высота старта (используется в cloudbase_msl) | Lenggries = 700m |
| `peaks` | Высота окружающих вершин (baseline для margin) | Lenggries = 1800m |

---

## 4. Thermal Window Detection

Из hourly profile (09:00+) выбираются часы, где **одновременно**:
- W* ≥ 1.0 м/с
- precipitation ≤ 0.5 мм
- cloudbase_msl − peaks ≥ 800 м

Результат: `thermal_window` с полями:
- `start` / `end` — первый и последний термический час
- `duration_h` — количество таких часов (`tw_hours`)
- `peak_hour` — час с максимальным lapse_rate (при равенстве — максимальный CAPE)
- `peak_lapse` / `peak_cape` — значения в peak_hour

**`tw_hours` — главный драйвер базового score в v2.0.**

---

## 5. Flyable Window

Самый длинный **непрерывный** отрезок часов (09:00–18:00), где **одновременно**:
- precipitation ≤ 0.5 мм
- windgusts_10m ≤ 12 м/с
- windspeed_10m ≤ 8 м/с

Результат: `continuous_flyable_hours`, `flyable_start`, `flyable_end`.

Flyable window ≠ thermal window.
Flyable = можно лететь (не сдует, не зальёт). Thermal = есть потоки для XC.

---

## 6. Флаги

Флаги генерируются из hourly profile по фиксированным порогам.
Каждый флаг = `(tag, message)`. Разделены на 4 категории.

### 6.1 Critical (вес −3 за каждый)

| Флаг | Условие | Агрегация |
|------|---------|-----------|
| `SUSTAINED_WIND_850` | mean(windspeed_850hPa) > 5.0 м/с | среднее по окну 09–18 |
| `GUSTS_HIGH` | max(windgusts_10m) > 10.0 м/с | максимум по окну 09–18 |
| `PRECIP_13` | precipitation @13:00 > 0.5 мм | точка 13:00 |
| `NO_FLYABLE_WINDOW` | continuous_flyable_hours = 0 | compute_flyable_window |

### 6.2 Low Base (вес −2)

| Флаг | Условие | Агрегация |
|------|---------|-----------|
| `LOW_BASE` | min(cloudbase_msl) − peaks < 1000 м | минимум по окну 09–18 |

### 6.3 Quality (вес −1 за каждый)

| Флаг | Условие | Агрегация |
|------|---------|-----------|
| `OVERCAST` | cloudcover @13:00 > 80% | точка 13:00 |
| `STABLE` | mean(lapse_rate) < 5.5 °C/km | среднее по окну 09–18 |
| `SHORT_WINDOW` | 0 < continuous_flyable_hours < 5 | compute_flyable_window |
| `GUST_FACTOR` | max(gust_factor) > 7.0 м/с | максимум по окну 09–18 |

### 6.4 Danger (вес −1 за каждый)

| Флаг | Условие | Агрегация |
|------|---------|-----------|
| `HIGH_CAPE` | max(cape) > 1500 J/kg | максимум по окну 09–18 |
| `CAPE_RISING` | late_cape > early × 1.5 **и** late > 800 | mean(capes[-2:]) vs mean(capes[:2]), мин. 4 точки |
| `VERY_UNSTABLE` | lifted_index @13:00 < −4 | точка 13:00 (только GFS) |

Примечание: `CAPE_RISING` генерируется только если уже есть `HIGH_CAPE`.

### 6.5 Дополнительные флаги (добавляются hard rules, v2.0)

| Флаг | Источник | Описание |
|------|----------|----------|
| `LOW_BASE_HARD` | Hard Rule 3 | min base < 2000m MSL → max MAYBE |
| `MODEL_DISAGREE` | Hard Rule 4 | Одна+ модель → NO-GO/UNLIKELY |
| `LOW_CONFIDENCE` | Hard Rule 5 | Model agreement LOW |
| `ENS_WIND_SPREAD` | Hard Rule 6 | Ensemble wind spread > 5 м/с |
| `ENS_CAPE_SPREAD` | Hard Rule 6 | Ensemble CAPE spread > 1000 J/kg |
| `MP_WEAK_THERMALS` | MP integration | Meteo-Parapente: max thermal < 0.3, hours ≤ 1 |
| `MP_STRONG_THERMALS` | MP integration | (positive) max ≥ 1.5 м/с, hours ≥ 3 |

---

## 7. Позитивные индикаторы (вес +1 за каждый, v2.0 — снижен с +2)

| Флаг | Условие | Агрегация |
|------|---------|-----------|
| `STRONG_LAPSE` | max(lapse_rate) > 7.0 °C/km | максимум по окну 09–18 |
| `GOOD_CAPE` | 300 < max(cape) < 1500 J/kg | максимум по окну 09–18 |
| `DEEP_BL` | max(boundary_layer_height) > 1500 м | максимум по окну (GFS) |
| `HIGH_BASE` | max(cloudbase_msl) − peaks > 1500 м | максимум по окну 09–18 |
| `LONG_WINDOW` | continuous_flyable_hours ≥ 7 | compute_flyable_window |
| `CLEAR_SKY` | cloudcover @13:00 < 30% | точка 13:00 |
| `GOOD_WSTAR` | max(W*) ≥ 1.5 м/с | максимум по окну (GFS) |
| `STRONG_SUN` | max(shortwave_radiation) > 600 W/m² | максимум по окну 09–18 |

---

## 8. Model Agreement

Сравнение **лучшей доступной ECMWF** vs **лучшей доступной ICON** at 13:00 local.

В v2.0 модели определяются динамически (какая успела в fallback chain):

```
Best ECMWF: ecmwf_ifs025 → ecmwf_ifs04 → ecmwf_hres(legacy)
Best ICON:  icon_d2 → icon_eu → icon_global → icon_seamless(legacy)
```

### Сравниваемые параметры и допуски

| Параметр | Допуск |
|----------|--------|
| temperature_2m | ±2.0 °C |
| windspeed_10m | ±2.0 м/с |
| windgusts_10m | ±3.0 м/с |
| cloudcover | ±20% |
| precipitation | ±0.5 мм |
| cape | ±200 J/kg |
| windspeed_850hPa | ±2.0 м/с |

### Формула

```
agreement_score = count(agree) / count(total)
```

### Confidence

| Score | Confidence |
|-------|-----------|
| ≥ 0.80 | HIGH |
| ≥ 0.50 | MEDIUM |
| < 0.50 | LOW |
| нет данных | UNKNOWN |

---

## 9. Ensemble Uncertainty

Для каждого ансамбля (ECMWF ENS, ICON-EU EPS) at 13:00 local —
по параметрам `temperature_2m, windspeed_10m, windgusts_10m, cloudcover, precipitation, cape, windspeed_850hPa`:

- `p10`, `p50`, `p90`, `spread` (= p90 − p10)

Большой spread → понижение статуса через hard rules 6.

---

## 10. Формула Score (v2.0 — REDESIGNED)

### 10.1 Base Score: Thermal Window

Основной вклад в score теперь — **размер термического окна** (`tw_hours`):

| tw_hours | base_score | Логика |
|----------|------------|--------|
| 0 | −6 | Нет термиков → NO-GO territory |
| 1–2 | −2 | Слабые термики → UNLIKELY |
| 3–4 | +1 | Приемлемый день → MAYBE+ |
| 5–6 | +4 | Хороший день → GO |
| 7+ | +6 | Отличный день → STRONG territory |

### 10.2 Deductions (вычитаются из base_score)

```
score = base_score
      - 3 × n_critical
      - 2 × n_low_base
      - 1 × n_quality
      - 1 × n_danger
```

### 10.3 Bonuses

```
score += 1 × n_positive    (каждый позитив = +1, снижено с +2 в v1.0)
```

### 10.4 Пороги score → status

| Диапазон | Status |
|----------|--------|
| ≤ −5 | NO-GO |
| −4 … −2 | UNLIKELY |
| −1 … 1 | MAYBE |
| 2 … 4 | GO |
| ≥ 5 | STRONG |

### 10.5 Примеры v2.0

| Ситуация | tw_hours | base | Deductions | Positives | Score | Status (до hard rules) |
|----------|----------|------|------------|-----------|-------|----------------------|
| Big XC day | 8 | +6 | 0 | 5×(+1) | +11 | STRONG |
| Хороший день, ветерок | 6 | +4 | WIND_850(−3) | 3×(+1) | +4 | GO |
| Средний день | 4 | +1 | OVERCAST(−1) | 2×(+1) | +2 | GO |
| Слабый | 3 | +1 | STABLE(−1), SHORT_WINDOW(−1) | 1×(+1) | 0 | MAYBE |
| Плохой | 2 | −2 | GUSTS_HIGH(−3), LOW_BASE(−2) | 1×(+1) | −6 | NO-GO |
| Нет термиков | 0 | −6 | OVERCAST(−1) | 0 | −7 | NO-GO |

---

## 11. Hard Rules (понижение статуса)

Score-based статус может быть **понижен** жёсткими правилами.
Повышение невозможно. Правила применяются **в указанном порядке**.

### Rule 1: Множественные critical → NO-GO

```
IF n_critical ≥ 2  OR  (n_critical ≥ 1 AND n_low_base ≥ 1):
    status = "NO-GO"
```

### Rule 2: Один critical при хорошем score → MAYBE

```
IF n_critical ≥ 1  AND  status IN ("GO", "STRONG"):
    status = "MAYBE"
```

### Rule 3 (v2.0): Низкая база → max MAYBE ⭐

```
IF cloudbase_min < 2000 m MSL  AND  status IN ("GO", "STRONG"):
    status = "MAYBE"
    + флаг LOW_BASE_HARD
```

Логика: база ниже 2000m = нет серьёзного XC. Даже если все остальные параметры
идеальны — MAYBE максимум.

### Rule 4 (v2.0): Per-model disagreement → worsen ⭐

```
bad_models = модели с assess_per_model().status IN ("NO-GO", "UNLIKELY")

IF bad_models  AND  status IN ("GO", "STRONG"):
    score -= len(bad_models)
    + флаг MODEL_DISAGREE

    IF len(bad_models) >= 2:
        status = "UNLIKELY"
    ELSE:
        status = "MAYBE"
```

Работает только если есть per_model_assessments (т.е. ≥2 модели).

### Rule 5: Низкая модельная уверенность → MAYBE

```
IF model_agreement.confidence == "LOW"  AND  status IN ("GO", "STRONG"):
    status = "MAYBE"
    + флаг LOW_CONFIDENCE
```

### Rule 6: Большой ансамблевый разброс → MAYBE

Для каждого ансамбля:

```
IF wind_spread > 5 м/с  AND  status IN ("GO", "STRONG"):
    status = "MAYBE"  +  ENS_WIND_SPREAD

IF cape_spread > 1000 J/kg  AND  status IN ("GO", "STRONG"):
    status = "MAYBE"  +  ENS_CAPE_SPREAD
```

### Rule 7: Нет данных

```
IF n_critical == 0  AND  n_quality == 0  AND  n_positive == 0  AND  tw_hours == 0:
    status = "NO DATA"
```

### Порядок применения

1. Score → базовый статус
2. Rule 1 (множественные critical → NO-GO)
3. Rule 2 (один critical + GO/STRONG → MAYBE)
4. Rule 3 (base < 2000m → MAYBE) ⭐ v2.0
5. Rule 4 (per-model disagree → worsen) ⭐ v2.0
6. Rule 5 (LOW confidence → MAYBE)
7. Rule 6 (ensemble spread → MAYBE)
8. Rule 7 (нет данных → NO DATA)

---

## 12. Per-Model Assessment (v2.0) ⭐

Помимо combined assessment, для каждой доступной детерминистической модели
строится **упрощённая оценка** через `assess_per_model()`:

### 12.1 Модели для per-model assessment

Все, что сохранены в `sources` с `_hourly_raw`:

```
icon_d2, icon_eu, icon_global, ecmwf_ifs025, ecmwf_ifs04, gfs_seamless
(+ legacy: ecmwf_hres, icon_seamless, gfs)
```

### 12.2 Упрощённая логика per-model

Для каждой модели независимо вычисляются:
- `flyable_hours` = `compute_flyable_window(model_profile)`
- `thermal_hours` = `_detect_thermal_window(model_profile, loc).duration_h`
- Quick checks: wind_850 mean > 5, precip @13 > 0.5

| Условие | Per-model status |
|---------|-----------------|
| precip @13 > 0.5 OR flyable = 0 OR high_wind | NO-GO |
| thermal ≤ 2 OR flyable < 4 | UNLIKELY |
| thermal ≤ 4 | MAYBE |
| thermal > 4 | GO |

### 12.3 Влияние на общий score

Per-model assessments влияют ТОЛЬКО через Hard Rule 4:
- Если модель дала NO-GO или UNLIKELY — score понижается на 1 за каждую такую модель
- Если ≥2 модели → status UNLIKELY
- Если 1 модель → status MAYBE

Это предотвращает ситуацию, когда combined profile (best-of-all) показывает GO,
но одна из моделей по отдельности даёт NO-GO.

---

## 13. Meteo-Parapente Integration (v2.0) ⭐

### 13.1 Когда вызывается

`integrate_meteo_parapente()` вызывается **после** headless scraper merge,
когда данные MP уже доступны в `result["sources"]["meteo_parapente"]`.

### 13.2 Какие данные извлекаются

Из перехваченного API ответа (`data.php` → JSON):
- `ths` — профиль термальной скорости по высотам (м/с)
- `z` — высотная шкала
- `pblh` — высота пограничного слоя

### 13.3 Вычисляемые метрики

| Метрика | Описание |
|---------|----------|
| `mp_max_thermal_ms` | Максимальная термическая скорость за окно 09–17 |
| `mp_thermal_top_m` | Максимальная высота с восходящим потоком |
| `mp_thermal_hours` | Количество часов с thermal ≥ 0.5 м/с |
| `mp_pblh_max_m` | Максимальная высота PBL |

### 13.4 Влияние на score

| Условие | Действие |
|---------|----------|
| max_thermal ≥ 1.5 AND thermal_hours ≥ 3 | +1 score, positive `MP_STRONG_THERMALS` |
| max_thermal < 0.3 AND thermal_hours ≤ 1 | −1 score, flag `MP_WEAK_THERMALS`, GO/STRONG → MAYBE |

---

## 14. Локации и их peaks

| Локация | Elev (launch) | Peaks (ceiling ref) | Margin 1000m = base MSL |
|---------|---------------|---------------------|-------------------------|
| Lenggries | 700 м | 1800 м | ≥ 2800 м |
| Wallberg | 1620 м | 1722 м | ≥ 2722 м |
| Kössen | 590 м | 1900 м | ≥ 2900 м |
| Innsbruck | 578 м | 2600 м | ≥ 3600 м |
| Greifenburg | 600 м | 2800 м | ≥ 3800 м |
| Speikboden | 950 м | 2500 м | ≥ 3500 м |
| Bassano | 130 м | 1700 м | ≥ 2700 м |

`peaks` определяет минимально необходимую базу облаков для XC.
`base_margin = cloudbase_msl − peaks`. Отрицательный margin = лететь невозможно.

Hard Rule 3 (v2.0): `cloudbase_min < 2000m` → max MAYBE — это **абсолютный** порог,
не привязанный к peaks. Даже в Bassano (peaks=1700) база 1900m = max MAYBE.

---

## 15. Ограничения текущей реализации

| Ограничение | Влияние |
|-------------|---------|
| W* зависит от GFS | Без GFS — нет W*, thermal window = 0 → base score = −6 |
| LI/CIN только GFS | Без GFS — нет VERY_UNSTABLE |
| Нет детекции фёна | Южный поток + сухой воздух на 700hPa не определяется |
| Scoring не учитывает тип XC | Одна шкала для всех дистанций |
| Per-model assessment упрощён | 4 уровня вместо 6, нет полного flag analysis |
| MP integration зависит от scraper | Без Docker / --no-scraper — нет MP данных |
| Thermal window gaps | Непрерывность не проверяется — считаются все подходящие часы |
