...
HTTP-запросы выполняются через retry-policy:
- до 3 попыток на transient ошибки сети (URLError/OSError/timeout),
- линейный backoff 2s, 4s,
- каждая неудачная попытка и факт повтора явно печатаются в консоль (`stderr`),
- HTTP 4xx считаются non-retryable и печатаются как финальная ошибка без повторов.
